...
This ebook covers the internal ballistics of solid propellant rocket
motors, with emphasis on performance modeling, grain design, and static
test anchoring.
...
Having defined propellant properties (Chapter 2), grain geometry
(Chapter 3), and how the burning surface evolves with web distance
(Chapter 4 --- burnback), we now combine these to predict the
time-dependent behavior of a solid rocket motor.

This chapter introduces the \textbf{zero-dimensional (0D) internal
ballistics model} --- the standard engineering approach for preliminary
design and performance prediction. It assumes spatially uniform pressure
...
rate from propellant burning - ( \dot{m}}\text{out} ): mass expulsion
rate through the nozzle - ( \rho\_p ): propellant density (kg/m³) - (
A\_b(t) ): instantaneous burning surface area (m²) from burnback - (
r(P\_c) ): linear burn rate (m/s)
...
where: - ( a ): burn rate coefficient - ( n ): pressure exponent
(typically 0.2--0.7 for stable motors) - ( P\_c ): chamber pressure
...
where ( A\_t ) is the throat area and ( c\^{}* ) is the characteristic
velocity of the propellant.

\section{Governing Differential
Equation}\label{governing-differential-equation}

Substituting and simplifying yields the core 0D ODE for chamber
pressure:
...
In practice we solve the coupled system for ( P\_c(t) ) and web distance
( w(t) ), with ( A\_b(w) ) supplied by the burnback module and numerical
integration (e.g., \texttt{scipy.integrate.solve\_ivp}).

\section{Key Assumptions}\label{key-assumptions}
...
\item
  Negligible erosive burning and ignition transients (added in later
  chapters)
\end{itemize}

\section{Summary}\label{summary}

This chapter establishes the fundamental differential equations linking
grain regression to motor performance. The next chapter explores the K/N
ratio and equilibrium pressure in greater depth.
...
\item
  ...
  Elements} (9th ed.). Wiley.
\item
  Greatrix, D. R. (2012). \emph{Powered Flight -- The Engineering of
  Aerospace Propulsion}. Springer.
\item
  Zucrow, M. J., \& Hoffman, J. D. (1976). \emph{Gas Dynamics} (Vol. 1).
  Wiley.
\item
  Wimpress, R. N. (1950). \emph{Internal Ballistics of Solid-Fuel
  Rockets}. McGraw-Hill.
\end{itemize}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Executable Python implementations and example simulations are provided
in the \texttt{code/} directory.
...
In the standard 0D internal ballistics model presented in Chapters 5 and
6, the burning rate depends only on chamber pressure (and initial
temperature) through Vieille's law: ( r = a P\^{}n ).

However, in many real motors --- especially those with high
length-to-diameter (L/D) ratios, narrow ports, or high mass flux --- the
hot combustion gases flowing parallel to the burning surface can
significantly \textbf{increase} the local burning rate. This phenomenon
is called \textbf{erosive burning}.

Erosive burning is most pronounced near the aft end of the grain, where
the core flow velocity (and thus mass flux) is highest due to cumulative
mass addition from upstream burning surfaces. It can lead to:
...
This chapter introduces the physical mechanism behind erosive burning
and presents the most widely used engineering model: the
\textbf{Lenoir-Robillard correlation}.

\section{Physical Mechanism}\label{physical-mechanism}

Erosive burning is primarily a \textbf{heat transfer} phenomenon. In
normal (non-erosive) combustion:

\begin{itemize}
\tightlist
\item
  Heat feedback to the propellant surface comes mainly from the thin
  primary combustion zone (flame) just above the surface via conduction,
  convection, and radiation.
\item
  The burning rate is limited by how quickly the propellant reaches
...
This effect becomes noticeable only above a certain \textbf{threshold
velocity} or mass flux, below which the normal pressure-dependent
burning dominates.

\section{Lenoir-Robillard Model}\label{lenoir-robillard-model}

The most common and practical engineering model for erosive burning was
proposed by Lenoir and Robillard (1957). It expresses the \textbf{total
burning rate} ( r ) as the sum of two contributions:

\[
r = r_0 + r_e
\]

where: - ( r\_0 = a P\_c\^{}n ): the normal (pressure-dependent) burning
rate - ( r\_e ): the additional erosive burning rate
...
where: - ( G = \frac{\dot{m}}{A_p} ): local mass flux in the port
(kg/m²·s), with ( A\_p ) the port area - ( D ): hydraulic diameter of
the port (m) - ( \rho\_p ): propellant density (kg/m³) - ( \alpha,
\beta ): empirical constants specific to the propellant (determined from
experiments)

The exponential term represents the \textbf{blowing effect} --- the
outward flow of pyrolysis gases from the burning surface reduces the
effectiveness of the convective heat transfer (transpiration cooling).
At very high burning rates or low mass flux, this term suppresses
erosive burning.

Many implementations use a \textbf{threshold} below which ( r\_e = 0 ):
...
\section{Implementation in the 0D
Model}\label{implementation-in-the-0d-model}

When erosive burning is included, the burn rate in the governing ODE
(Chapter 5) becomes position- and time-dependent:

\[
r(x, t) = a P_c^n + r_e(G(x, t))
\]

where ( G(x, t) = \frac{\dot{m}(x, t)}{A_p(x, t)} ) and ( \dot{m}(x, t)
) is the cumulative mass flow from the head end up to axial position ( x
).

This turns the simple lumped 0D model into a \textbf{quasi-1D} model,
requiring integration along the grain length (or at least several
ballistic elements).
...
\item
  Average or integrate to update the effective ( A\_b r ) term in the
  mass balance.
\end{itemize}

\section{Design Implications and
Mitigation}\label{design-implications-and-mitigation}
...
\tightlist
\item
  Port area is small relative to throat area (high ( K\_n ))
\item
...
\tightlist
\item
  Increasing initial port diameter or using progressive geometries
  (star, finocyl) that open up quickly
\item
  Designing for lower average ( K\_n )
\item
  Using propellants with higher burn rate or lower erosive sensitivity
...
In extreme cases, erosive burning can cause the aft end of the grain to
burn through prematurely, leading to nozzle erosion or case overheating.

\section{Summary}\label{summary-2}

Erosive burning augments the normal pressure-dependent burn rate due to
enhanced convective heat transfer from high-velocity core flow. The
Lenoir-Robillard model provides a practical semi-empirical way to
include this effect:
...
Incorporating erosive burning improves the accuracy of pressure-time
predictions, especially for high-performance or high L/D motors. It is
typically added after the basic 0D model is working and validated
against non-erosive cases.

In the next chapter we will examine \textbf{ignition transients}, the
initial pressurization phase that occurs before steady-state burning is
established.
...
\tightlist
\item
  Lenoir, J. M., \& Robillard, G. (1957). A Mathematical Method to
  Predict the Effects of Erosive Burning in Solid Propellant Rockets.
  \emph{Sixth Symposium (International) on Combustion}.
\item
  Sutton, G. P., \& Biblarz, O. (2017). \emph{Rocket Propulsion
  Elements}.
\item
  ...
  Aerospace Propulsion}.
\item
  Nakka, R. Solid Rocket Motor Design Resources (practical
  implementation notes).
\end{itemize}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

The Python implementation in \texttt{code/simulator.py} will include an
optional \texttt{erosive=True} flag that applies a Lenoir-Robillard
correction based on local mass flux. Propellant-specific ( \alpha ) and
( \beta ) values will be stored in the propellant database.
